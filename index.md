...
- [`tools/graph.py`](tools/graph.py) , query + validate the typed-edge graph (the `relations:` layer; see AGENTS.md). `python3 tools/graph.py --node <name>` shows a concept's inbound/outbound edges; `--validate` checks integrity; `--html` builds the interactive viewer.
